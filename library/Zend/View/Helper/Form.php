...
class Zend_View_Helper_Form extends Zend_View_Helper_FormElement
{
    ...
    public function form($name, $attribs = null, $content = false)
    {
        ...
        if (is_array($attribs) && array_key_exists('id', $attribs) && empty($attribs['id'])) {
            unset($attribs['id']);
        }
        ...
        if ($this->_isHtml5() && is_array($attribs) && array_key_exists('action', $attribs) && !$attribs['action']) {
            unset($attribs['action']);
        }

        if ( is_array($attribs) && array_key_exists('name', $attribs) && empty($attribs['id'])) {
            unset($attribs['id']);
        }
