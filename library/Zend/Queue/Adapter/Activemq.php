...
class Zend_Queue_Adapter_Activemq extends Zend_Queue_Adapter_AdapterAbstract
{
    ...
    public function __construct($options, Zend_Queue $queue = null)
    {
        ...
        if (!is_array($options) || !array_key_exists('scheme', $options)) {
            $options['scheme'] = self::DEFAULT_SCHEME;
        }
        if (!is_array($options) || !array_key_exists('host', $options)) {
            $options['host'] = self::DEFAULT_HOST;
        }
        if (!is_array($options) || !array_key_exists('port', $options)) {
            $options['port'] = self::DEFAULT_PORT;
        }

        if (is_array($options) && array_key_exists('stompClient', $options)) {
